...
import { Input, Button, Typography, Space, Row, Col, notification } from 'antd';
...
export default function CreateTodoForm() {
  ...
  useEffect(() => {
    if (state.message === 'Created') {
      api.success({ message: 'Todo created successfully' });
      const timeout = setTimeout(() => {
        router.push('/todo');
      }, 800);
      return () => clearTimeout(timeout);
    } else if (state.message && state.message !== 'Created') {
      api.error({ message: state.message || 'Something went wrong' });
    }
